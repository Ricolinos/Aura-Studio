...
**Versión 1 — 2026-08-17 (D-293 en `Aura-Firmware`, ST-012 en Aura Studio).** Copia idéntica en ambos repositorios: `Aura-Firmware/docs/contracts/library-layout-v1.md` es la fuente canónica; Aura Studio la copia a `docs/contracts/library-layout-v1.md`. Todo cambio futuro sube la versión de este documento **y** el campo `version` del marcador de §4, en los dos repos, en la misma unidad de trabajo — misma convención que `CONTRATO-firmware-studio.md` (que lo referencia desde su §D).
...
| `/Videos/` | **Plano**, sin subcarpetas: `<archivo>.mpg`/`.mpeg` (MPEG-2, `mpegplayer`). Póster opcional `<archivo>.jpg` hermano (mismo nombre base) | Studio | Firmware (`aura_video.c`, `opendir("/Videos")`, filtro por extensión) |
