...
**`.rockbox/aura/artists/<archivo>.jpg`** — la foto en sí. JPEG baseline, **cuadrada**, lado ≤ 128px (mismas reglas de formato que D.1: sRGB, sin progresivo, sin ICC). **v18: Studio garantiza el recorte cuadrado centrado desde v(ST-14x); antes enviaba el lado mayor a 128 px con la proporción original, lo que violaba esta cláusula. El firmware sigue tolerando cualquier proporción con fill-crop (§D.5).** Nombre = el mismo algoritmo que ya usa Studio para su caché local (`ArtistImageStore.fileName(forArtistKey:)`: alfanuméricos ASCII y `-` tal cual, espacio → `-`, resto → `_%02x`, recortado a 120 caracteres + `.jpg`) — determinístico, no hace falta que el firmware lo calcule, solo lo busca por el nombre que dice el índice.
