...
**Versión 20 — 2026-09-05.** Copia idéntica en ambos repositorios (`Aura-Firmware` es la fuente canónica; Aura Studio la referencia como "copia de la versión N de este contrato"). Cualquier cambio se hace en los dos repos en la misma unidad de trabajo y sube el número de versión.

**v20 (foto de artista, misma resolución que la carátula de álbum).** `.rockbox/aura/artists/<archivo>.jpg` pasa de "cuadrada, lado ≤ 128 px" a **cuadrada, 320×320 px, JPEG baseline sRGB, sin ICC, sin progresivo, calidad 0.85** — el mismo formato que ya rige `cover.jpg` desde v1.5 de `docs/contracts/library-layout-v1.md` (D-349, contrato v18 del firmware). Los tres firmwares siguen aceptando fotos de bibliotecas anteriores con lado ≤ 128 px (fill-crop y ampliación como hasta ahora, §D.5) — no hay ruptura de compatibilidad hacia atrás, solo una resolución de origen mayor para las nuevas. **§D.5 no cambia**: la caché MAESTRA de foto de artista sigue derivándose a 130×130 (es el lado más grande que cualquier familia necesita para su propio uso salvo una excepción puntual: el fondo de "Ahora suena" de Metro-Aura/moonlit.aura decodifica la fuente de 320 px directamente, sin pasar por la maestra, cuando necesita más resolución que 130 px para ese fondo — ver el firmware de cada familia). Studio reescribe las fotos de artista existentes a 320×320 en la primera sincronización que corra tras Aura Studio 0.2.1; hasta entonces, los usuarios con fotos ya sincronizadas siguen viendo las de 128 px sin ningún error. Aura-Firmware no cambia código en esta versión: sus círculos de 48 px en la lista de Artistas siguen saliendo de la caché maestra de 130×130, sin tocar el archivo fuente directamente.
...
**v17 (ST-077 — instalar desde cero usa el Release MÁS NUEVO, no el pin) — solo lado Studio; ningún firmware cambia.** Hasta v16, `FIRMWARE_VERSION` (§E) era la **única** fuente de instalación: los binarios viajan embebidos en la app, puestos ahí al compilar por `scripts/fetch-firmware.sh`. Eso dejaba dos síntomas que el dueño reportó (2026-08-27): la pastilla de versión de Extras mostraba el pin aunque hubiera un Release más nuevo publicado, y **una instalación desde cero escribía la versión del pin**, no la más reciente — todo esto mientras el aviso de actualizaciones (ST-074, con el token del Llavero) ya sabía perfectamente que existía algo más nuevo. Desde v17:

- **§A no cambia en absoluto.** La lista de assets de un Release (`rockbox.ipod`, `rockbox.zip`, `bootloader-ipod6g.ipod`, `mks5lboot`, `checksums.txt`) y su verificación por SHA-256 contra `checksums.txt` son exactamente las mismas. Lo que cambia es **cuándo** Studio las consume: antes solo al compilar, ahora también al instalar.
- **§E: el pin pasa a ser el RESPALDO, no la única vía.** Studio baja los cinco assets del Release más nuevo de la familia elegida a `~/Library/Application Support/AuraStudio/firmware-cache/<familia>/<tag>/`, los verifica igual que a los embebidos (hashes + el contenido real de `rockbox.zip`, D-297/D-298) y **recién entonces** instala desde ahí. Si cualquier paso falla — sin red, token sin acceso, Release incompleto, checksum que no cuadra —, instala lo embebido **y lo dice en pantalla**. Nunca se queda sin instalar; nunca escribe algo sin verificar.
- **El bootloader y `mks5lboot` salen del MISMO Release que el árbol.** Flashear el bootloader de una versión y copiar el árbol de otra sería una mezcla que ningún release probó nunca.
- **Descarga de assets de un repo privado**: por la URL del **API** del asset (`/repos/:owner/:repo/releases/assets/:id`) con `Accept: application/octet-stream`, nunca por `browser_download_url` — esa redirige a un host de almacenamiento que rechaza la cabecera `Authorization`. En el salto del 302 hay que soltar esa cabecera.
- **`firmware-version.txt`** se escribe junto a los artefactos descargados con el tag exacto, para que la pantalla de Licencias (§B) y el manifiesto de instalación (v11) sigan citando la versión que de verdad se instaló.

Lo que v17 **no** cambia: **§D entero** (nada de lo que se escribe en el disco del iPod cambia de ruta ni de formato), ni §A, ni el bootloader, ni nada que ningún firmware lea o escriba. Los tres firmwares no tienen trabajo en esta versión.
...
- **Versión de formato y purga (v18)**: `/.aura/art/format.txt` contiene un entero decimal (v18: `2`). Al arrancar, cada familia lee el archivo; si falta o el valor es menor que el suyo, borra **todo** `/.aura/art/{albums,artists,photos}` y sus cachés L2 privadas (`.rockbox/aura/cfcache`, `.rockbox/aura/photocache`, `moonlitcache/`, `/.aura/thumbs/`) y escribe su versión. Studio nunca lo toca ni lo borra (igual que el resto de `/.aura/art`).
...
- **§E**: `FIRMWARE_VERSION` lleva una sección por familia (sin prefijo / `metro.` / `moonlit.`); `fetch-firmware.sh --family aura|metro|moonlit`. Desde **v17** ese pin es el **respaldo**: instalar baja primero el Release más nuevo de la familia y lo verifica igual.
...
| `.rockbox/aura/artists/<archivo>.jpg` | Studio (sync, **OPCIONAL**) | Firmware (`aura_artist_images.c`) | v6, `PLAN-biblioteca-medios-v2.md` → **v20**: 320×320 px (antes ≤128 px; fotos viejas se siguen aceptando). Contrato detallado en **D.3** abajo. Ausente = placeholder circular con ícono, degradación soportada |
...
**`.rockbox/aura/artists/<archivo>.jpg`** — la foto en sí. JPEG baseline, **cuadrada**, **320×320 px, sRGB, sin ICC, sin progresivo, calidad 0.85** (v20: mismo formato exacto que `cover.jpg` desde v1.5 de `docs/contracts/library-layout-v1.md` — antes de v20, lado ≤ 128 px). **v18: Studio garantiza el recorte cuadrado centrado desde v(ST-14x); antes enviaba el lado mayor a 128 px con la proporción original, lo que violaba esta cláusula.** El firmware sigue tolerando **cualquier** proporción y **cualquier** tamaño con fill-crop y ampliación (§D.5) — fotos de bibliotecas anteriores a v20, con lado ≤ 128 px, se siguen leyendo sin error; la primera sincronización tras Aura Studio 0.2.1 reescribe las fotos de artista existentes al nuevo tamaño. Nombre = el mismo algoritmo que ya usa Studio para su caché local (`ArtistImageStore.fileName(forArtistKey:)`: alfanuméricos ASCII y `-` tal cual, espacio → `-`, resto → `_%02x`, recortado a 120 caracteres + `.jpg`) — determinístico, no hace falta que el firmware lo calcule, solo lo busca por el nombre que dice el índice.
...
- Álbum: ruta de la pista representativa (la primera que la búsqueda por álbum devuelve) y su `mtime` según la base de datos de música. **v18: el `<mtime>` de la clave `a-<crc32>.<mtime>` es `max(mtime de la pista representativa, mtime de `cover.jpg` hermano si existe)`. Así una carátula reescrita sin tocar la pista invalida la maestra (cierra la hipótesis (a) de D-338/M-096/D-055).**
...
- (v14) El cambio entre **cualquier par** de familias desde el dispositivo (Ajustes › Cambiar sistema) queda implementado en las tres: Aura D-333, Metro-Aura M-093, moonlit.aura D-047. Lo que sigue pendiente es solo del lado Studio: recuperación con dos o más dormidos (pedir elegir). (v18: `--family moonlit` en `fetch-firmware.sh` sale de esta lista — ya está implementado, `scripts/fetch-firmware.sh:33,164`.)
