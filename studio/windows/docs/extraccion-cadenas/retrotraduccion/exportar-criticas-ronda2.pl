...
#
#   perl exportar-criticas-ronda2.pl <cultura> [<cultura>...]
#
...
#
# Los ids son nuevos y empiezan en d001 — la 'd' de esta ronda, para que ni él
# ni nadie confunda una fila d012 de ahora con la c012 de antes.
#
...
my %already;
my $first = 'docs/extraccion-cadenas/retrotraduccion/mapa-criticas.csv';
open(my $mh, '<:encoding(UTF-8)', $first) or die "$first: $!";
<$mh>;
while (my $line = <$mh>) { chomp $line; my (undef, $key) = split(/,/, $line, 2); $already{$key} = 1 if $key }
close $mh;
die "el mapa de la primera ronda está vacío\n" unless %already;
...
my %id;
$id{$critical[$_]} = sprintf("d%03d", $_ + 1) for 0 .. $#critical;

my $dir = 'docs/extraccion-cadenas/retrotraduccion';
open(my $map, '>:encoding(UTF-8)', "$dir/mapa2-criticas.csv") or die $!;
print $map "id,clave\n";
print $map "$id{$_},$_\n" for @critical;
close $map;
print "escrito: $dir/mapa2-criticas.csv (" . scalar(@critical) . " claves)\n";

for my $culture (@ARGV) {
    my $translated = values_of("AuraStudio.Core/Strings/Resources.$culture.resx");
    my $path = "$dir/criticas2-$culture.csv";
